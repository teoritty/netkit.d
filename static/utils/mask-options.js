// Builds the subnet mask options (CIDR + dotted-mask).
...
export function buildMaskOptions(selected = 24) {
  return MASKS.map(([value, mask]) => {
    const isSelected = Number(value) === selected ? ' selected' : '';
    return `<option value="${value}"${isSelected}>/${value} (${mask})</option>`;
  }).join('');
}
